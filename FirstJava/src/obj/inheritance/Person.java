 ...
 *	- 부모(super) 클래스의 멤버(변수와 method)를 자식(sub) 클래스에서 사용이 가능함
 *	- 생성자는 상속이 안됨.
 * 	- Sub Class "is a" Super Class	(is a == 상속)
 ...
 * 	- Object 클래스는 최상위 클래스
 * 	- final(상수) 클래스는 상속 불가...
 * 	- abstract(추상) 클래스는 반드시 상속해서 사용해야 함
 * 
 ...
 *  	 -> 일반화 된 클래스는 부모(super)클래스
 *  	 -> 구체화 된 클래스는 자식(sub) 클래스
 *  	 -> extends 키워드를 사용하게 됨...
 * 
 */
...
public class Person {
	...
	public Person(String name, int age, float height, float weight) {	// 속성값을 초기화 (필드 생성자)
		this.name = name;
		this.age = age;
		this.height = height;
		this.weight = weight;
	...
	public void walk() {	//메서드
		System.out.println(name+ "는 걸어가요.");
	}
	
	//Object 메서드 오버라이드 hashCode()	- // 객체 동등 비교를 위한 값을 생성하는 메서드 !!! 
	@Override
	public int hashCode() {
	...
	public boolean equals(Object obj) {	
		...
		Person other = (Person)obj;	//() 타입캐스팅을 통해 비교할 수 있도록 함
		if(age != other.age) return false;	//객체 내에 age 멤버의 값을 비교확인
		if(name == null)
			if(other.name != null) {
				return false;	//객체 내에 이름을 확인
