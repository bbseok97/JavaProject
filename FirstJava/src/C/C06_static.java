/*
 * 	Usage Modifier
 * 	-Usage Modifier는 Access Modifier를 제외한 모든 Modifier를 의미함.	// (private default, protected, public)을 제외한 모든 modifier.
 ...
 *  	그리고 static이 붙지 않는 변수를 인스턴스 변수라고 하며 구분합니다.
 * 	- 메서드 static이 붙는다면 해당 메서드는 인스턴스와 무관하기 때문에 인스턴스를 생성하지 않아도
 *		호출이 가능하게 된다.
 *
 * 	-- static 관련된 자바 문법
 *		> static 메서드 안에서는 non-static 멤버를 객체 생성없이 직접 참조할 수 없음.
 * 		> 반대로 static 붙지 않은 메서드에서는 static 멤버를 사용할 수 있음.
 * 
 * 	- 주로 모든 인스턴스가 공통된 변수값을 가져야 하는 경우가 있는데 이때 static 변수를 사용합니다.
 * 		static 변수와 관련된 작업을 하거나 어떤 인스턴스 변수도 사용하지 않고, 매개변수로만 작업을
 * 		드로 정의해 줍처리하는 메서드는 static 메서드다.
 * 		ex) Math 클래스의 메서드들...
 *		 
 */
package C;
