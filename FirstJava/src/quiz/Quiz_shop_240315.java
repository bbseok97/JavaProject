...
public class Quiz_shop_240315 {
			 ...
			 */
				...
				Product[] trunk = new Product[MAX_TRUNK_SIZE];	//트렁크 사이즈
				int t_index = 0;
				
				//카트
				final int INIT_CART_SIZE = 3;
				Product[] cart = new Product[INIT_CART_SIZE];	//카트 사이즈
				int c_index = 0;
		...
		int getNullCount() {	//카트 안에 비어있는 내용을 세기 위해
			int count = 0;
			for(int i = 0; i < c_index; i++) {
		...
		void cancel(int cart_index) {
			...
			/*  5. 매서드명 : receipt
			 * 	     기능 : 바이어가 구매한 물건의 목록과 사용 금액과 남은 금액을 출력한다. 
			 *     리턴타입, 매개변수 : 없음
			 */
		...
		void receipt() {
			...
			System.out.println("남은 금액 : " + cash); 	// 남은 금액
		}
		...
		// 아무것도 적지 않은 생성자에게는 super()가 생략되어 있음...
