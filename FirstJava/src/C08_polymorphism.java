 ...
 * 	- 다형성은 존재하지 않았던 클래스를 **기존 시스템에 수정없이, 추가되도록 합니다.
 * 		(다형성 인자 에제)
 * 	- 다형성을 적용하는 방법은 부모클래스의 객체를 선언하고, 자식클래스의 객체를 할당하는 것.
 * 		반대의 경우에는 불가능 ! (큰 범위 -> 작은범위 O / 작은범위->큰범위 X)
 * 	- 다형적 객체의 경우 부모 클래스에 있는 메서드만 사용이 가능함.
 * 		자식 클래스에 재정의 되어 있다면 실행은 자식클래스에 있는 메서드로 실행됩니다.
 * 		자식클래스의 메서드가 실행된다고 해서 부모 클래스에 있는 해당 메서드를 지우면 안됩니다.
...
public class C08_polymorphism {
	...
	public static void main(String[] args) {
		// 다형성 테스트
		Person p = new Student("홍길동", 17, "20240315", "도술" );	//upCasting
...
//		Student casting = (Student)p;	//downCasting
//		// study()를 사용해야 하는 경우에는 Student로 캐스팅이 되어야 사용이 가능해요.
//		casting.study();
//		
//		/////
//		Person p2;
//		p2 = new Teacher ("홍선생", 22, "Java Programing!!!");
//		// instanceof 연산자는 좌항의 객체와 우항의 클래스를 비교!	(참/거짓)
