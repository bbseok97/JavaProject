...
public class G16_optionalEx {
	...
	/*
	 * 	집계 메서드
	 * 	Stream API에서 요소들의 최소값, 최대값, 합계, 평균값 갯수 등을 구할 수 
	 * 	있는 메서드를 제공하고 있음.	합계와 갯수를 구해주는 메서드 이외 Optional로 시작하는 타입으로 리턴.
	 * 
	 */
	public static void main(String[] args) {
		...
		//	OptionalXXX, isPresent() - isPresent() 값이 있으면 true / isEmpty()는 값이 없으면 true 
		OptionalDouble avg2 = list2.stream().mapToInt(Integer::intValue).average();
		//	요소값이 있는 경우에만 출력
		if(avg2.isPresent()) {
		System.out.println("요소들의 평규 : " +avg2.getAsDouble());	//오류 발생 위치
		}
		...
		list2.stream().mapToInt(Integer::intValue).findFirst()
										.ifPresent(a -> System.out.println("요소의 첫번째 값 : " +a));		//ifPresent : 값이 존재하는 경우
		
		
		
		//	사용자 집계 메서드	- reduce() 메서드
		//	사용자가 다양한 집계 결과물을 생성할 수 있는 메서드
		//	reduce()는 매개변수 타입이 XXXOperator를 이용하고, 
