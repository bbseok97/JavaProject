...
public class G09_streamEx2 {
	...
	/*
	 *	Stream은 BaseStream 인터페이스를 최상위로 Stream(Object), IntStream(int), 
	 *	LongStream(long), DoubleStream(double)을 가지고 있다.
	...
	public static void main(String[] args) {
		...
		IntStream intstream3 = IntStream.range(1, 6);		// startNum : inclusive,	endNum : exclusive
		intstream3.forEach(s -> System.out.print(s + "\t"));
		System.out.println();
		
		System.out.println("IntStream.rangeClosed(1, 5) IntStream 생성");
		IntStream intstream4 = IntStream.rangeClosed(1, 5);		//startNum : inclusive,		endNum : inclusive
		intstream4.forEach(s -> System.out.print(s + "\t"));
		System.out.println();
		// 난수 Stream 생성하기
		System.out.println("int형 난수 스트림 : ");
		IntStream isr = new Random().ints(3);			//int(3)은 3개의 정수 난수를 생성
		...
		// Stream 객체 생성
		isr = str.chars();		// why? IntStream을 사용했을까?	char는 정수 계산이 가능하기 때문에
